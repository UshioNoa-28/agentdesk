...
class StreamHubTests(IsolatedAsyncioTestCase):
    ...
    async def test_publish_routes_by_session_and_reports_missing_queue(self) -> None:
        hub = StreamHub()
        queue = hub.register("s1")

        self.assertTrue(await hub.publish("s1", TextDelta(content="hello")))
        self.assertFalse(await hub.publish("missing", TextDelta(content="lost")))
        self.assertEqual(("text_delta", {"content": "hello"}), await queue.get())
    ...
    async def test_close_wakes_consumers_and_rejects_new_registrations(self) -> None:
        hub = StreamHub()
        queue = hub.register("s1")
        ...
        self.assertIsNone(hub.register("s2"))
        self.assertFalse(await hub.publish("s1", TextDelta(content="late")))
