...
from agent.infrastructure.metatools import SearchMcpTool
from agent.prompt.system import build_system_prompt

_REAL_ASYNC_CLIENT = httpx.AsyncClient


def _mcp_file(base: Path, content: str) -> Path:
    """在 ``base`` 下落盘一个 ``.agent-desk/mcp.json`` 测试夹具。"""

    (base / ".agent-desk").mkdir(parents=True, exist_ok=True)
    (base / ".agent-desk" / "mcp.json").write_text(content, encoding="utf-8")
    return base


class McpConfigTests(TestCase):
    def test_repository_example_ships_route_ready_descriptions(self) -> None:
        with TemporaryDirectory() as directory:
            example = Path(__file__).parents[1] / ".agent-desk" / "mcp.json.example"
            _mcp_file(Path(directory), example.read_text(encoding="utf-8"))
            config = load_mcp_configuration(Path(directory), home=Path(directory))

        self.assertEqual(("exa-search",), tuple(server.id for server in config.servers))
        self.assertTrue(all(len(server.description) > 100 for server in config.servers))
        self.assertTrue(config.servers[0].enabled)
        prompt = build_system_prompt(tuple((s.id, s.description) for s in config.servers))
        self.assertIn('server: "exa-search"', prompt)

    def test_stdio_entry_defaults_type_and_timeout(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"local": {"description": "local", '
                '"command": "python", "args": ["-s"]}}}',
            )
            server = load_mcp_configuration(Path(directory), home=Path(directory)).servers[0]

        self.assertEqual("stdio", server.transport)
        self.assertEqual(30.0, server.timeout_seconds)
        self.assertTrue(server.enabled)
        self.assertEqual(("-s",), server.args)

    def test_streamable_http_is_an_http_alias(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"rag": {"type": "streamable-http", "description": "rag", '
                '"url": "http://rag/mcp"}}}',
            )
            server = load_mcp_configuration(Path(directory), home=Path(directory)).servers[0]
    ...
    def test_memory_aliases_normalize_to_in_memory(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"py": {"type": "memory", "description": "in-proc"}}}',
            )
            server = load_mcp_configuration(Path(directory), home=Path(directory)).servers[0]

        self.assertEqual("in-memory", server.transport)

    def test_description_is_required_for_routing(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(Path(directory), '{"mcpServers": {"x": {"command": "python"}}}')
            config = load_mcp_configuration(Path(directory), home=Path(directory))

        self.assertEqual((), config.servers)

    def test_unknown_entry_field_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"x": {"description": "d", "command": "python", '
                '"autoApprove": ["a"]}}}',
            )
            config = load_mcp_configuration(Path(directory), home=Path(directory))
    ...
    def test_url_without_type_is_an_error_not_a_stdio_guess(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"x": {"description": "d", "url": "http://a/b"}}}',
            )
            config = load_mcp_configuration(Path(directory), home=Path(directory))

        self.assertEqual((), config.servers)

    def test_unsupported_type_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"x": {"type": "sse", "description": "d", '
                '"url": "http://a/b"}}}',
            )
            config = load_mcp_configuration(Path(directory), home=Path(directory))
    ...
    def test_stdio_headers_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"local": {"description": "d", "command": "python", '
    ...
    def test_missing_top_level_mcp_servers_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(Path(directory), '{"servers": {}}')
            config = load_mcp_configuration(Path(directory), home=Path(directory))

        self.assertEqual((), config.servers)

    def test_non_json_syntax_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(Path(directory), "mcpServers: {}\n")
            config = load_mcp_configuration(Path(directory), home=Path(directory))

        self.assertEqual((), config.servers)

    def test_explicit_empty_registry_loads_zero_servers(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(Path(directory), '{"mcpServers": {}}')
            config = load_mcp_configuration(Path(directory), home=Path(directory))
    ...
    def test_absent_config_file_loads_zero_servers(self) -> None:
        with TemporaryDirectory() as directory:
            config = load_mcp_configuration(Path(directory), home=Path(directory))
    ...
    def test_http_headers_expand_from_environment(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"exa": {"type": "http", "description": "Exa", '
    ...
    def test_http_headers_expand_from_dotenv_without_process_environment(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            with patch(
                ...
                try:
                    server = load_mcp_configuration(
                        Path(directory), home=Path(directory)
                    ).servers[0]
                finally:
    ...
    def test_unset_environment_variable_without_default_drops_file(self) -> None:
        with TemporaryDirectory() as directory:
            _mcp_file(
                Path(directory),
                '{"mcpServers": {"exa": {"type": "http", "description": "Exa", '
                '"url": "https://mcp.exa.ai/mcp", '
                '"headers": {"x-api-key": "${AGENTDESK_TEST_UNSET_KEY}"}}}}',
            )
            with patch(
                "agent.infrastructure.mcp.config.load_dotenv_environment",
                return_value={},
    ...
    def test_broken_nearer_file_does_not_block_farther_layer(self) -> None:
        with TemporaryDirectory() as directory:
            home = Path(directory) / "home"
            project = home / "work" / "repo"
            project.mkdir(parents=True)
            _mcp_file(project, "{ not json")
            _mcp_file(
                home,
                '{"mcpServers": {"far": {"description": "far", "command": "far-cmd"}}}',
            )

            config = load_mcp_configuration(project, home=home)

        self.assertEqual(("far",), tuple(server.id for server in config.servers))

    def test_nearer_file_shadows_same_name_and_merges_other_servers(self) -> None:
        with TemporaryDirectory() as directory:
            home = Path(directory) / "home"
            project = home / "work" / "repo"
            project.mkdir(parents=True)
            _mcp_file(
                project,
                '{"mcpServers": {"shared": {"description": "near", "command": "near-cmd"}}}',
            )
            _mcp_file(
                home,
                '{"mcpServers": {"shared": {"description": "far", "command": "far-cmd"}, '
                '"extra": {"description": "extra", "command": "python"}}}',
            )

            config = load_mcp_configuration(project, home=home)

            servers = {server.id: server for server in config.servers}
            self.assertEqual(("shared", "extra"), tuple(s.id for s in config.servers))
            self.assertEqual("near", servers["shared"].description)
            self.assertIn("extra", servers)
...
class McpRegistryTests(IsolatedAsyncioTestCase):
    ...
    async def test_failed_server_does_not_prevent_other_server_startup_and_can_retry(self) -> None:
        ...
        with self.assertLogs("agent.infrastructure.mcp.registry", level="WARNING") as logs:
            await registry.start_all_async()
        ...
        self.assertTrue(
            any("bad" in line and "failed to connect" in line for line in logs.output),
            msg=logs.output,
        )
