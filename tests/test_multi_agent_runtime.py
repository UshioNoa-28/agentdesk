...
from agent.infrastructure.metatools.workspace import ReadFileTool
from agent.infrastructure.runtime import AgentFactory, AgentRuntimeManager
...
def _failure(payload: dict[str, Any]) -> dict[str, Any]:
    """取失败信封里的 error 对象。"""
...
class MultiAgentManagerTests(IsolatedAsyncioTestCase):
    ...
    def _manager(self, scopes: FakeRuntimeScopes) -> AgentRuntimeManager:
        return AgentRuntimeManager(
            ...
            factory=AgentFactory(
                scopes=scopes, status_registry=InMemoryAgentStatusRegistry()
            ),
            scopes=scopes,
    ...
    async def test_subagent_cannot_dispatch_task_to_peer(self) -> None:
        """星形拓扑下沉到领域：子代理发起的 SubAgentMessage 构造即被拒。"""

        with self.assertRaises(MessageRoutingError) as raised:
            SubAgentMessage(
                sender="coder",
        ...
        self.assertEqual("star_topology_violation", raised.exception.code)
    ...
    async def test_main_message_rejects_empty_session_envelope(self) -> None:
        with self.assertRaises(DomainValidationError):
            MainAgentMessage(sender="user", content="hi", session_id=" ")
...
class DefineSubagentToolTests(IsolatedAsyncioTestCase):
    ...
    def test_define_subagent_requires_a_well_formed_name(self) -> None:
        # name 的非空与字符集由 schema 声明，工具内不再判第二次。
        for arguments in (
            {"name": "", "description": "d", "system_prompt": "p"},
            {"name": "has space", "description": "d", "system_prompt": "p"},
            ...
            with self.subTest(arguments=arguments):
                with self.assertRaises(ValidationError):
                    validate(
                        instance=arguments,
                        schema=dict(DEFINE_SUBAGENT_DEFINITION.parameters),
                    )
...
class SendMessageToolTests(IsolatedAsyncioTestCase):
    async def test_dispatch_is_fire_and_forget_with_agent_identity(self) -> None:
        ...
        called_msg: SubAgentMessage = mock_runtime.dispatch.call_args[0][0]
    ...
    async def test_subagent_report_carries_subagent_identity(self) -> None:
        ...
        self.assertTrue(_payload(res)["ok"])
        called_msg: MainAgentMessage = mock_runtime.dispatch.call_args[0][0]
    ...
    async def test_subagent_cannot_message_another_subagent(self) -> None:
        ...
        self.assertEqual("star_topology_violation", _failure(payload)["code"])
    ...
    async def test_execution_failure_is_reported_as_tool_error(self) -> None:
        ...
        self.assertEqual("dispatch_failed", error["code"])
        self.assertIn("boom", str(error["message"]))
...
class ListSubagentsToolTests(IsolatedAsyncioTestCase):
    ...
    async def test_surfaces_live_status_per_subagent(self) -> None:
        """注册表里的实时状态随每个子代理一并返回。"""
        main_sess = Session.create(title="main")
        coder_sess = Session.create(
            title=f"{main_sess.id}_subagent_coder", main_session_id=main_sess.id
        ...
        # reviewer 不设置：模拟"已定义但当前不在跑"。

        list_tool = ListSubagentsTool(
            session_service=session_service, status_registry=registry
        )
        listed = _payload(
            await list_tool.aexecute(
                {}, context=ToolContext(caller_session_id=main_sess.id)
            )
        )

        by_name = {item["name"]: item["status"] for item in listed["subagents"]}
        self.assertEqual("running", by_name["coder"])
        self.assertEqual("idle", by_name["reviewer"])

    async def test_falls_back_to_idle_when_no_live_record(self) -> None:
        """注册表里没该子代理记录（从未实例化）时状态回退为 idle。"""
        main_sess = Session.create(title="main")
        coder_sess = Session.create(
            title=f"{main_sess.id}_subagent_coder", main_session_id=main_sess.id
        )
        ...
        registry = InMemoryAgentStatusRegistry()  # 空：coder 无记录
        list_tool = ListSubagentsTool(
            session_service=session_service, status_registry=registry
        )
        listed = _payload(
        ...
        self.assertEqual("idle", listed["subagents"][0]["status"])
...
class MetaToolRegistryTests(IsolatedAsyncioTestCase):
    ...
    def test_get_tools_filters_by_allowed_tools_whitelist(self) -> None:
        granted = _build_registry().get_tools(("read_file", "send_message"))
        granted_names = [t.definition.name for t in granted]
        self.assertEqual(["read_file", "send_message"], granted_names)
