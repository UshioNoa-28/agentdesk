...
class WaitForRepliesToolTests(IsolatedAsyncioTestCase):
    ...
    def test_non_numeric_timeout_is_rejected_by_the_schema(self) -> None:
        # 类型与下界是 schema 契约；工具只负责缺省回填与向配置上限夹取。
        with self.assertRaises(ValidationError):
            validate(
                instance={"timeout_seconds": "nonsense"},
                schema=dict(WAIT_FOR_REPLIES_DEFINITION.parameters),
            )
        with self.assertRaises(ValidationError):
            validate(
