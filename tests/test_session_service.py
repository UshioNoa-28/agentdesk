        ...
        *,
        ...
        if self.sessions is not None:
            ...
            if session is None:
                return None
        current = max(
            (m.seq for m in self.items.get(session_id, ())),
            default=0,
        )
        if current > 0:
            seq = current + 1
        elif session is not None and session.parent_last_seq is not None:
...
class SessionServiceTests(IsolatedAsyncioTestCase):
    ...
    async def test_create_session_does_not_persist_default_system_prompt(self) -> None:
        """默认 system prompt 是活配置，创建会话不落任何 SYSTEM 行；每轮由 ContextManager 现拼。"""
    ...
    async def test_resumed_session_inherits_single_parent_system_prompt(self) -> None:
        # 自定义 persona 属于会话本体：入库后 resume 链上重放一次且不重复。
        parent = await self.service.create(
            title="Parent",
            custom_system_prompt="You are a persona.",
        )
        ...
        self.assertEqual("You are a persona.", system_msgs[0].content)
    ...
    async def test_resume_creates_child_session(self) -> None:
        ...
        await self._append(parent.id, "old")           # seq 1 = user
        await self._append(parent.id, "answer")        # seq 2 = user
    ...
    async def test_history_uses_parent_and_child_messages(self) -> None:
        ...
        self.assertEqual(child_message.id, history[2].id)
    ...
    async def test_resumed_session_seq_continues_monotonically_from_parent(self) -> None:
        parent = await self.service.create(title="Parent")
        await self._append(parent.id, "first user query")  # seq 1
        await self._append(parent.id, "second user query") # seq 2
        child = await self.service.resume(session_id=parent.id)
        child_msg1 = await self._append(child.id, "child query 1")
        child_msg2 = await self._append(child.id, "child query 2")

        self.assertEqual(3, child_msg1.seq)
        self.assertEqual(4, child_msg2.seq)

        history = await self.service.history(child.id)
        self.assertEqual([1, 2, 3, 4], [m.seq for m in history])
    ...
    async def test_resumed_session_history_is_isolated_from_subsequent_parent_messages(
        ...
        self.assertEqual(4, len(c_history))

        # 9. 验证 P 的历史消息：严格包含 [G1, G2, P1, P2]，不含 G3, G4
        ...
        self.assertEqual(4, len(p_history))
    ...
    async def test_create_subagent_session_allows_worker_tools(self) -> None:
        ...
        sub_sess = await self.service.create(
            ...
            main_session_id=main_sess.id,
            custom_system_prompt="prompt",
            allowed_tools=("send_message", "edit_file"),
        )
        self.assertEqual(("send_message", "edit_file"), sub_sess.allowed_tools)
    ...
    async def test_create_session_with_allowed_tools_persists_and_resumes(self) -> None:
        ...
        self.assertEqual(("edit_file", "send_message"), sess.allowed_tools)

        fetched = await self.service.get(sess.id)
        self.assertEqual(("edit_file", "send_message"), fetched.allowed_tools)

        resumed = await self.service.resume(session_id=sess.id)
        self.assertEqual(("edit_file", "send_message"), resumed.allowed_tools)
    ...
    async def test_resume_with_custom_parent_last_seq(self) -> None:
        ...
        await self._append(parent.id, "Message 1")  # seq 1
        await self._append(parent.id, "Message 2")  # seq 2
        await self._append(parent.id, "Message 3")  # seq 3

        # 分叉到 seq 2（保留 Message 1 和 Message 2）
        ...
        self.assertEqual(2, child.parent_last_seq)
        history = await self.service.history(child.id)
        self.assertEqual(2, len(history))  # Message 1 + Message 2
