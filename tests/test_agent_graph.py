...
from agent.domain.tools import AgentTool, ToolDefinition, ToolResult
from agent.infrastructure.metatools import ExecuteMcpTool, SearchMcpTool
...
class AgentGraphTests(IsolatedAsyncioTestCase):
    ...
    async def test_calls_beyond_per_turn_cap_get_skipped_and_paired(self) -> None:
        """超过单轮并行上限的 call 不执行，但必须合成配对结果落库。"""
        persisted: list[tuple[ModelMessage, dict[str, object]]] = []
        after_model, after_tool = _record(persisted)
        hook_registry = AgentHookRegistry()
        hook_registry.register_after_model(after_model)
        ...
        graph = AgentGraph(
            max_turns=4,
            # 单轮并行上限就是调度参数里的固定数字：上限 2，第三条被 skip。
            settings=ContextSettings(max_tool_calls_per_turn=2),
            hook_registry=hook_registry,
            permission_service=_AllowPermissions(),
        )
        result = await graph.ainvoke(
            messages=[ModelMessage.human("three")],
            model=_ThreeCallModel(),
            tools=(_ExampleTool("execute_mcp"),),
            caller_session_id="test-session",
        )
        ...
        tool_messages = [message for message, _ in persisted if message.role == "tool"]
        self.assertEqual(
            ("a", "b", "c"), tuple(message.tool_call_id for message in tool_messages)
        )
        self.assertEqual("hello A", tool_messages[0].content)
        self.assertEqual("hello B", tool_messages[1].content)
        skipped = json.loads(tool_messages[2].content)
        self.assertFalse(skipped["ok"])
        self.assertEqual("too_many_tool_calls", skipped["error"]["code"])
        self.assertEqual("done", result["messages"][-1].content)
    ...
    async def test_ask_user_continues_same_turn_and_reconsults_model(self) -> None:
        """重构后 ask_user 不再终止本轮：结果回填后 after_tool→before_model,模型再被调一次。"""
        ...
        self.assertEqual(2, model.turn)
        last = result["messages"][-1]
        self.assertEqual("assistant", last.role)
        self.assertEqual("第二轮继续回答", last.content)
    ...
    async def test_ask_user_mixed_batch_runs_all_calls_then_continues(self) -> None:
        """同轮混调:ask_user 之后的调用照常执行完;本轮继续,模型再被调一次收尾。"""
...
class _AskThenContinueModel:
    """第一轮调 ask_user；本轮不结束,after_tool 回 before_model,故第二轮被再调一次。"""
    ...
    async def ainvoke(
        ...
        return ModelTurn(message=ModelMessage.assistant(content="第二轮继续回答"), tool_calls=())


class _AskMixedWithAnotherModel:
    """一次回复里 ask_user 在前、普通工具在后：两者都执行完后本轮继续,第二轮再被调。"""
...
class _ThreeCallModel(_TwoCallModel):
    async def ainvoke(
        self, *, messages, tools, tool_choice=None, max_output_tokens=None
    ) -> ModelTurn:
        self.turn += 1
        if self.turn == 1:
            calls = (
                ToolCall(id="a", name="execute_mcp", arguments={"name": "A"}),
                ToolCall(id="b", name="execute_mcp", arguments={"name": "B"}),
                ToolCall(id="c", name="execute_mcp", arguments={"name": "C"}),
            )
            return ModelTurn(
                message=ModelMessage.assistant(content="", tool_calls=calls),
                tool_calls=calls,
