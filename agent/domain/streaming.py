...
messages 模式捕获，经进程内 StreamHub 直达入口消费者，不落库、不参与
持久化。
"""
...
from agent.domain.interruption import AskUserRequest, PermissionRequest
...
# Agent 流（跨 actor 边界到达入口消费者）的事件集合；assistant 发起工具调用时
# 直接复用领域 ToolCall 值对象，人机中断复用领域 InterruptionRequest。
AgentStreamEvent = Union[
    TextDelta, ToolCall, ToolResultEvent, PermissionRequest, AskUserRequest
]

# 流事件名：队列里流动的帧就是 (事件名, JSON 载荷)，名称与 CLI 渲染
# 分发一一对应，改任何一边都要同步另一边。
STREAM_EVENT_TEXT_DELTA = "text_delta"
STREAM_EVENT_TOOL_CALL = "tool_call"
STREAM_EVENT_TOOL_RESULT = "tool_result"
STREAM_EVENT_PERMISSION_REQUEST = "permission_request"
STREAM_EVENT_ASK_USER_REQUEST = "ask_user_request"
...
def stream_frame(event: AgentStreamEvent) -> StreamFrame:
    ...
    if isinstance(event, PermissionRequest):
        # wire 键 permission_id / permission_request 是 CLI 既有契约，保持不变。
        return STREAM_EVENT_PERMISSION_REQUEST, {
            "permission_id": event.interruption_id,
    ...
    if isinstance(event, AskUserRequest):
        return STREAM_EVENT_ASK_USER_REQUEST, {
...
__all__ = [
    "STREAM_EVENT_ASK_USER_REQUEST",
    "STREAM_EVENT_ERROR",
    "STREAM_EVENT_MESSAGE_END",
    "STREAM_EVENT_PERMISSION_REQUEST",
    "STREAM_EVENT_QUEUED",
