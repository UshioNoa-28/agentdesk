...
入口消费者注册队列并读取，队列的摘除交给本轮 ``ask`` 任务。两条路径互不阻塞，
...
class AgentStreamSink(Protocol):
    ...
    唯一实现：``agent.infrastructure.runtime.stream_hub.QueueStreamSink``（由
    ``StreamHub.sink_for`` 创建，包裹入口侧注册的 asyncio.Queue）。
    """
...
class AgentStreamHubPort(Protocol):
    ...
    队列元素是 ``(事件名, JSON 载荷)`` 帧，``None`` 是消费者约定的
    ...
    async def publish(self, session_id: str, event: AgentStreamEvent) -> bool:
        """向 session 当前队列发布事件；没有活跃队列时返回 ``False``。"""
