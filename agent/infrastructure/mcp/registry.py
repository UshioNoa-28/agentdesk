...
from agent.domain.tools import ToolDefinition, ToolResult, error_result
from agent.infrastructure.bm25_tokenizer import tokenize
    ...
    async def _start_server(self, server_id: str, config: McpServerConfig) -> McpServerStatus:
        async with self._server_locks[server_id]:
            ...
            try:
                ...
                # 连接真失败时给一条干净 warning（宿主按黄色展示），而不是静默——
                # 结构化的 FAILED 状态与 retry_async 重连入口本就保留在注册表里。
                logger.warning("MCP server '%s' failed to connect: %s", server_id, exc)
