...
生命周期约定：``register`` 归入口消费者所有，``remove`` 归本轮
``ask`` 任务所有——队列跟着那一轮图生死，不跟着连接生死，所以"注册存在"
的含义是"这一轮还在跑"，与还有没有人在读无关。普通图事件仍可通过
``sink_for`` 发布；不方便持有 sink 的运行期中断组件（例如 InterruptionBroker）通过
``publish(session_id, event)`` 发布。同一 session 同时只允许一条活跃流。

队列里流动的是线格式帧 ``(事件名, JSON 载荷)``；领域事件在发布端经
...
class StreamHub:
    ...
    def sink_for(self, session_id: str) -> AgentStreamSink | None:
        ...
        queue = self._queues.get(session_id.strip())
        return None if queue is None else QueueStreamSink(queue)

    async def publish(self, session_id: str, event: AgentStreamEvent) -> bool:
        """向当前队列发布一个事件，返回是否找到活跃队列。

        ``put_nowait`` 只在当前事件循环内执行，不会在查找队列和入队之间
        让出控制权，因此不会把一个已移除的队列误认为仍然可用。
        """
        ...
        if queue is None:
            return False
        try:
            queue.put_nowait(stream_frame(event))
        except asyncio.QueueFull:
            logger.warning("Stream queue is full: session_id=%s", session_id)
            return False
        return True
    ...
    def close(self) -> None:
        ...
        logger.debug("Stream hub closed: queues=%s", len(queues))
