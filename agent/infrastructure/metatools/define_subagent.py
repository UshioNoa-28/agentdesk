...
from agent.infrastructure.metatools.execute_mcp import EXECUTE_MCP_DEFINITION
from agent.infrastructure.metatools.load_skill import LOAD_SKILL_DEFINITION
from agent.infrastructure.metatools.search_mcp import SEARCH_MCP_DEFINITION
from agent.infrastructure.metatools.send_message import SEND_MESSAGE_DEFINITION
...
GRANT_TOOL_DEFINITIONS: dict[str, ToolDefinition] = {
    ...
    "search_text": SEARCH_TEXT_DEFINITION,
...
class DefineSubagentTool(AgentTool):
    ...
    async def aexecute(
        ...
        *,
        ...
        # 保留名是 schema 未表达的授权策略：main_agent 可伪造主控身份绕过
        # 星形约束，user 会污染消息来源判定。
        ...
        allowed_tools: tuple[str, ...] = tuple(arguments.get("allowed_tools") or ())
        if not allowed_tools:
            allowed_tools = DEFAULT_SUBAGENT_TOOL_NAMES

        # 主控专用工具禁止下放给子代理。当前 allowed_tools 的 enum 恰好不含
        # 这些名字，此检查表达的是授权策略而非类型校验：一旦有人把主控工具
        # 加进可授予集合，星形约束不能随之失守。
        ...
        # 子代理实际拿不到能力，且全程无任何报错。会话也可经 HTTP 直接指定
        # allowed_tools，那条路径不经过本工具的 schema，故此处仍需自守。
