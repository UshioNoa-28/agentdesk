...
class WaitForRepliesTool(AgentTool):
    ...
    def __init__(
        ...
        *,
        ...
        max_timeout: float,
        session_service: SessionServicePort | None,
    ) -> None:
    ...
    async def aexecute(
        ...
        *,
        ...
        # 缺省由部署配置回填；上限同样来自配置，schema 的 minimum 已保证不为 0。
        timeout = (
            self._default_timeout
            if raw_timeout is None
            else min(float(raw_timeout), self._max_timeout)
        )
    ...
    def _result(waited_seconds: int, *, arrived: bool) -> ToolResult:
        ...
        return ok_result({"waited_seconds": waited_seconds, "note": note})
