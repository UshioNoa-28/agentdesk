...
<long_term_memory>
You have file-based long-term memory in two directories:
{memory_layers}
Each layer holds MEMORY.md (a pure index: one "- [title](title.md) —
description" line per note) plus one markdown note file per note, named by
its title (frontmatter written stamp, then the body). A missing file or
directory just means that layer has no memories yet. The index is NOT
injected here: when a task touches user preferences or project facts, read
the relevant MEMORY.md with read_file first, then open the matching notes.
Persist durable facts with the remember Meta Tool. Before writing, search
these directories for related notes; when they contradict, trust the one
with the newer "written" stamp and consolidate or delete the stale note.
</long_term_memory>
...
def build_system_prompt(
    ...
    *,
    ...
    """把 MCP 路由、Skill 元数据、Meta Tool 描述与记忆索引拼进稳定 system prompt。
    ...
    Args:
        ...
        memory_layers (str): 两层记忆目录的路径清单（见
            ``load_memory_prompt``）；空串时给出未配置的占位说明。
    ...
    return (
        ...
        .replace(
            "{memory_layers}",
            memory_layers.strip() or "(long-term memory is not configured)",
        )
    )
