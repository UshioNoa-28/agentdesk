...
class _BaseMessageProjector:
    ...
    def __init__(
        ...
        *,
        ...
        ``token_counter`` 与 ``settings`` 由组合根注入。投影帽子
        （单条工具结果截断上限、cold 清除阈值）是纯调度参数，来自全局
        注入的 ``ContextSettings``，与当前模型无关。
        """
        ...
        if kind == MessageKind.SKILL_RESULT:
            ...
            content = self.wrap_untrusted_content(
                self._token_counter.truncate_text(
                    message.content,
                    self._settings.max_tool_result_tokens,
                )
            )
    ...
    def _project_plain(message: Message, *, kind: MessageKind) -> ModelMessage:
        ...
        SUMMARY 在模型视角改写为 human 角色并包上对话摘要边界：摘要是
        模型生成的产物，但必须按"外部注入的现状"被读取——human 角色的
        指令权重高于模型自己的旧发言（codex/OpenAI 同款编码约定）。
        持久层角色不变，改写只发生在视图边界。
        """
    ...
    def _visible_messages(messages: Sequence[Message]) -> list[Message]:
        ...
        for index in range(len(ordered) - 1, -1, -1):
            ...
            if (
                ...
                continue
            boundary_index = positions.get(candidate.metadata.get("covered_through_message_id"))
            if boundary_index is None or boundary_index >= index:
                # 数据损坏 不然不可能 总结未来的信息
                continue
            # summary_message 的 index，与该 summary cover-through 的 index
            latest = (index, boundary_index)
            break
        ...
        tail = [
            ...
            and position != latest_index # 但是不能包括 summary本身 
    ...
    def _cleared_message_ids(self, messages: Sequence[Message]) -> frozenset[str]:
        ...
        for message in messages:
            ...
            if (
                ...
                continue
            boundary_index = positions.get(metadata.get("cold_cleared_through_message_id"))
            if boundary_index is None:
                continue
            checkpoint = (message, boundary_index)
        if checkpoint is None:
            return frozenset()
        _, boundary_index = checkpoint
        threshold = self._settings.clear_tool_result_threshold_tokens
        cleared: set[str] = set()
        for item in messages[: boundary_index + 1]:
            if (
                item.role == "tool"
                and message_kind_from_value(item.metadata.get("kind")) == MessageKind.TOOL_RESULT
                and self._token_counter.estimate_text(self.wrap_untrusted_content(item.content))
                > threshold
            ):
                cleared.add(item.id)
