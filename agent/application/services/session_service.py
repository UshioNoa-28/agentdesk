...
class SessionService(SessionServicePort):
    """负责 Session 元数据、自定义 System Prompt 持久化、resume 和删除。

    默认 system prompt 是活配置（MCP/Skill/工具授权/记忆目录），不在这里
    落库——每轮由 ContextManager 现拼；只有 define_subagent 写入的自定义
    persona 属于会话本体，作为 seq=1 SYSTEM 消息持久化。
    """
    ...
    async def create(
        ...
        *,
        ...
        """创建 Session；仅自定义 System Prompt 原子落库，默认 prompt 每轮现拼。"""
        ...
        async with self._unit_of_work:
            ...
            if custom_system_prompt is not None:
                system_message = Message.create(
                    ...
                    seq=1,
                    message=ModelMessage.system(custom_system_prompt.strip()),
